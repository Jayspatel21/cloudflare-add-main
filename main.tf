...
resource "aws_instance" "my_ec2_instance" {
  ...
  # Use remote-exec provisioner to upload environment files after instance is created
  provisioner "remote-exec" {
    inline = [
      "echo 'Uploading Firefly environment file' > /tmp/firefly.env",
      "echo '${var.firefly_env_content}' >> /tmp/firefly.env",
      "echo 'Uploading Dragonfly environment file' > /tmp/dragonfly.env",
      "echo '${var.dragonfly_env_content}' >> /tmp/dragonfly.env"
    ]
    ...
    connection {
      type        = "ssh"
      user        = "ec2-user"
      private_key = file(var.ssh_key_private)  # Path to private key for SSH
